...
import { shareTelegramReferralLink, shareReferral, isTelegramWebApp, getTelegramUser } from "../lib/telegram";
import {
...
export function WalletPage({
    ...
    if (effectiveAddress) {
      ...
      const unsubRewards = onValue(rewardRef, (snapshot) => {
        if (snapshot.exists()) {
          ...
          setPendingReferralRewards(totalEarnedUsGold || ((nCount + pCount) * 1) || 0);
        } else {
          setPendingCount(0);
  ...
  const handleClaimApprovedUsGold = async () => {
    ...
    try {
      ...
      // Add claimed amount to usGoldBalance and earnings in users/${effectiveAddress}
      ...
      if (userSnap.exists()) {
        ...
        currentEarnings = uData.earnings || 0;
        currentReferralEarnings = uData.referralEarnings || 0;
      }
      ...
      await update(userRef, {
        ...
        earnings: currentEarnings + amountToClaim,
        referralEarnings: currentReferralEarnings + amountToClaim,
  ...
  const handleShareReferral = async () => {
    triggerHaptic(15);
    const refAddress = effectiveAddress || 'GOLDEN';
    const res = await shareReferral(
      refAddress,
      t('stakingReserveTitle', language),
    ...
    const fetchSolBalance = async () => {
      if (effectiveAddress && connection) {
        try {
          ...
          console.warn("Error fetching SOL balance (possible RPC restriction):", e);
          if (isMounted) setSolBalance(0);
        }
      }
    };
        ...
        if (activeQuote && activeQuote.outAmount) {
          ...
          if (swapRes.ok) {
            ...
            if (swapData && swapData.swapTransaction) {
              ...
              txSignature = await executeSolanaTransaction({
                connection,
                transaction: vTx,
                sendTransaction,
                walletProvider,
      ...
      <Card sx={{
            ...
            >
              ...
              >
                <Stack direction="column" alignItems="flex-start" sx={{ textAlign: 'left', lineHeight: 1.1 }}>
                  ...
                  <Typography variant="body2" sx={{ fontSize: '11px', fontWeight: 900, color: '#fff', display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <TokenIcon symbol="usGOLD" size={14} />
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1.5 }}>
            <Typography variant="h3" fontWeight="900" sx={{ color: '#fff', fontSize: { xs: '2.5rem', sm: '3rem' } }}>
              ${(totalStaked + liveTotalAccrued).toFixed(2)}
            </Typography>
            <Typography variant="h6" fontWeight="800" sx={{ color: alpha('#fff', 0.4) }}>
              USD
            </Typography>
          </Box>
          <Typography variant="body2" sx={{ color: '#14F195', fontWeight: 700, mt: 0.5 }}>
        ...
        <Box 
          sx={{ 
            p: 2.5, 
            borderRadius: '20px', 
            bgcolor: approvedToClaimAmount > 0 ? alpha('#14F195', 0.1) : alpha('#fff', 0.03),
            border: `1px solid ${approvedToClaimAmount > 0 ? alpha('#14F195', 0.4) : alpha('#fff', 0.1)}`,
            display: 'flex',
            justifyContent: 'space-between',
            alignItems: 'center',
            flexWrap: 'wrap',
        ...
        >
          <Stack direction="row" spacing={2} alignItems="center">
            <Avatar sx={{ 
              bgcolor: approvedToClaimAmount > 0 ? alpha('#14F195', 0.2) : alpha('#fff', 0.05),
              color: approvedToClaimAmount > 0 ? '#14F195' : 'text.secondary',
              width: 48,
              height: 48
            }}>
              <Sparkles size={24} />
            ...
            <Box>
              <Typography variant="body1" fontWeight="900" sx={{ color: approvedToClaimAmount > 0 ? '#14F195' : '#fff', display: 'flex', alignItems: 'center', gap: 0.75 }}>
          ...
          <Button
            ...
            startIcon={isClaimingUsGold ? <CircularProgress size={18} color="inherit" /> : (approvedToClaimAmount > 0 ? <Download size={20} /> : <UserPlus size={20} />)}
            sx={{
              fontWeight: 900,
              px: 4,
              borderRadius: '14px',
              ...
              <Grid container spacing={3} alignItems="center">
                ...
                <Grid item xs={12} md={4}>
                  ...
                  >
                    ...
                    <Typography variant="caption" color={needsApprovalCount > 0 ? "#FFDF73" : approvedToClaimAmount > 0 ? "#14F195" : "#4caf50"} fontWeight="800">
                      {needsApprovalCount > 0
                        ? `🟡 ${(needsApprovalCount * 1).toFixed(2)} usGOLD ${t('approvedPendingAdmin', language) || 'Pending Admin Approval'}`
                        : approvedToClaimAmount > 0 
                          ? `🎁 ${approvedToClaimAmount.toFixed(1)} usGOLD ${t('approvedReadyToClaim', language)}` 
                          ...
                          <Box>
                            ...
                            {rw.stakeAmount && (
                              <Typography variant="caption" color="#D4AF37" fontWeight="bold">
                                Staked: ${parseFloat(rw.stakeAmount || 0).toFixed(2)} USD ({rw.stakeDurationMonths}m) | {rw.solPaid} SOL
                              </Typography>
                            )}
                          </Box>
                        </Stack>
