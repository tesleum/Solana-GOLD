    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        await update(userRef, {
          ...
          referralEarnings: (userData.referralEarnings || 0) + 1,
...
export function AdminStaking() {
  ...
  // Auto-check qualified staking referral users: if referee has staked, ensure reward exists as 'needs_approval' for 1 usGOLD approval
  useEffect(() => {
    if (!users || Object.keys(users).length === 0 || !stakes || stakes.length === 0) return;

    const checkAndSyncReferrals = async () => {
      const rewardMap = new Map<string, any>();
      rewards.forEach((r) => {
        if (r.referrerId && r.referee) {
          rewardMap.set(`${r.referrerId.toLowerCase()}_${r.referee.toLowerCase()}`, r);
        }
      });

      for (const refereeId of Object.keys(users)) {
        const userData = users[refereeId];
        const referrerId = userData?.referrer;
        if (!referrerId) continue;

        // Check if referee has staked
        const refereeStakes = stakes.filter(
          (s) => s.userAddress && s.userAddress.toLowerCase() === refereeId.toLowerCase()
        );
        if (refereeStakes.length === 0) continue;

        // Referee has active/completed stake(s) -> Qualified for 1 usGOLD referral reward
        const mapKey = `${referrerId.toLowerCase()}_${refereeId.toLowerCase()}`;
        const existingReward = rewardMap.get(mapKey);
        const sampleStake = refereeStakes[0];

        if (!existingReward) {
          // Push new reward with status 'needs_approval'
          const rewardRef = ref(database, `rewards/${referrerId}`);
          await push(rewardRef, {
            type: "referral_reward",
            amount: 1,
            referee: refereeId,
            status: "needs_approval",
            timestamp: sampleStake.createdAt || Date.now(),
            completedAt: sampleStake.createdAt || Date.now(),
            stakeAmount: sampleStake.amount || 0,
            stakeDurationMonths: sampleStake.durationMonths || 12,
            solPaid: sampleStake.solPaid || "0"
          });
        } else if (existingReward.status === "pending") {
          // Upgrade status from pending to needs_approval
          const rewardPath = `rewards/${existingReward.referrerId}/${existingReward.key}`;
          await update(ref(database, rewardPath), {
            status: "needs_approval",
            completedAt: sampleStake.createdAt || Date.now(),
            stakeAmount: existingReward.stakeAmount || sampleStake.amount || 0,
            stakeDurationMonths: existingReward.stakeDurationMonths || sampleStake.durationMonths || 12,
            solPaid: existingReward.solPaid || sampleStake.solPaid || "0"
          });
        }
      }
    };

    checkAndSyncReferrals();
  }, [users, stakes, rewards]);
                            ...
                            <Stack direction="row" spacing={0.5} alignItems="center">
                              ...
                              {row.referee && (
                                <IconButton
                                  size="small"
                                  onClick={() => {
                                    navigator.clipboard.writeText(row.referee);
                                    setCopiedAddress(row.referee);
                                    setTimeout(() => setCopiedAddress(null), 2000);
                                  }}
                                >
                                  {copiedAddress === row.referee ? <Check size={12} color="#4caf50" /> : <Copy size={12} color="#888" />}
                                </IconButton>
                              )}
                            </Stack>
                          </TableCell>
