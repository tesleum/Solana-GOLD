...
function Dashboard() {
  ...
  // Capture referrer and initialize Telegram Mini App on mount
  useEffect(() => {
    initTelegramIntegration();
  ...
  useEffect(() => {
    ...
    if (addressToRegister) {
      ...
      get(userRef).then((snapshot) => {
        if (!snapshot.exists()) {
          const urlParams = new URLSearchParams(window.location.search);
          const storedReferrer = getTelegramReferralParam() || urlParams.get('ref') || urlParams.get('start') || localStorage.getItem('referrer');
          ...
          const updateData: any = { lastActive: timestamp };
          if (tgUserId) {
      ...
      const unsubscribe = onValue(txRef, (snapshot) => {
        ...
        if (data) {
          ...
          const formattedTx = Object.keys(data).map(k => {
            ...
            if (match && txData.type !== 'vault_investment' && txData.type !== 'stake_created') {
  ...
  const handleInvestment = async () => {
    ...
    try {
      ...
      try {
        ...
        const totalSolNeeded = amountToInvest + (feeBuffer / LAMPORTS_PER_SOL);
        
        if (balanceSol < totalSolNeeded) {
          throw new Error(`Insufficient SOL balance. You need approximately ${totalSolNeeded.toFixed(4)} SOL (including fees).`);
        }
      } catch (balErr: any) {
        // If it's a balance error we threw, we re-throw it.
        if (balErr.message?.includes("Insufficient SOL balance")) {
          throw balErr;
        }
        // Otherwise it might be a Tatum restricted plan error, we log and proceed
        console.warn("Could not verify balance via RPC, proceeding with transaction:", balErr);
      }

      // Fetch MLM Settings
      const adminWallets = adminWalletsData || {};
      const adminWallet1Str = '6PCtQ1NeTdyPpBCVZv1NGCSaBaRy9UaYXNLdmdqEtz5a'; // Primary Pool
